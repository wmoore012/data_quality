...
def _validate_rules_table(cursor) -> bool:
    ...
    for col, expected_type in expected_columns.items():
        ...
        if not any(expected_part in columns[col].lower() for expected_part in expected_type.lower().split()):
            logger.warning(f"⚠️ Column type mismatch in data_quality_rules.{col}: expected {expected_type}, got {columns[col]}")
...
def _validate_results_table(cursor) -> bool:
    ...
    required_columns = [
        'result_id', 'scan_timestamp', 'rule_id', 'issue_count',
        'severity', 'table_name', 'column_name', 'details',
...
def _validate_thresholds_table(cursor) -> bool:
    ...
    required_columns = [
        'threshold_id', 'table_name', 'metric_name',
...
def _validate_constraints(cursor) -> bool:
    ...
    if not cursor.fetchone():
        logger.warning("⚠️ Missing foreign key constraint: data_quality_results -> data_quality_rules")
