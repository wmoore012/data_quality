        ...
        with engine.connect() as conn:
            ...
            if "isrc" in [col.lower() for col in columns]:
                ...
                null_result = conn.execute(
                    text(f"SELECT COUNT(*) FROM {table_name} WHERE isrc IS NULL")
                )
            ...
            if results["total_rows"] > 0:
                ...
                if null_check_cols:
                    null_condition = " AND ".join(
                        [f"{col} IS NULL" for col in null_check_cols[:5]]
                    )  # Check first 5 non - ID columns
...
def print_human_readable_report(
    table_results: List[Dict], mode: str, block_on_critical: bool
):
    ...
    for result in table_results:
        ...
        if issues:
            ...
            if critical_count > 0:
                print(
                    f"🔴 {table_name}: {row_count:,} rows, {issue_count} issues ({critical_count} critical)"
                )
            else:
        ...
        if result["sample_data"]:
            ...
            key_fields = [
                k
                for k in sample
                if k.lower() in ["id", "isrc", "title", "name", "spotify_id"]
            ][:3]
            if key_fields:
                sample_str = ", ".join(
                    [f"{k}={sample[k]}" for k in key_fields if sample[k] is not None]
                )
                if sample_str:
...
def main():
    ...
    parser.add_argument(
        "--database - url",
        help="Database URL (optional, will use production DB if not provided)",
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--tables", help="Comma - separated table names to check (optional)"
    )
    ...
    if args.json:
        ...
        success = print_human_readable_report(
            table_results, args.mode, args.block_on_critical
        )
