...
from data_quality.schema_analyzer import analyze_schema
...
def run_non_interactive_check(
    ...
    try:
        ...
        results = {
            "timestamp": "2024-12-17T10:00:00Z",  # Would be actual timestamp
            "database_url_masked": database_url.split('@')[-1] if '@' in database_url else "local",
            "scan_results": {
            ...
            "ci_cd_status": {
                "should_fail": fail_on_critical and report.summary.get("critical", 0) > 0,
...
def main():
    ...
    parser.add_argument(
        "--tables",
        help="Comma-separated table patterns to scan"
    )
    parser.add_argument(
        "--fail-on-critical",
        action="store_true",
    ...
    if args.fail_on_critical and results["ci_cd_status"]["should_fail"]:
        sys.exit(1)
