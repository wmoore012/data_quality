...
class SQLPolicyChecker(ast.NodeVisitor):
    ...
    def __init__(self) -> None:
        ...
        self.sql_execute_methods = {
            'execute', 'executemany', 'execute_many', 'exec_driver_sql'
        }
    ...
    def _is_unsafe_sql_construction(self, node: ast.AST) -> bool:
        ...
        if isinstance(node, ast.BinOp) and isinstance(node.op, ast.Add):
            if self._contains_variable(node.left) or self._contains_variable(node.right):
                return True
...
def check_sql_security(code: str, filename: str = "<string>") -> List[SQLSecurityViolation]:
    """
