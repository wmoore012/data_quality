...
from .safe_scanners import find_orphans, find_nulls_safe, find_duplicates_safe
...
@cli.command("check")
@click.option("--database-url", envvar="DATABASE_URL", required=True, help="Database connection URL")
@click.option("--format", type=click.Choice(["text", "json", "sarif"]), default="text", help="Output format")
@click.option("--fail-on", type=click.Choice(["none", "warning", "critical"]), default="critical", help="Exit code behavior")
    ...
    try:
        ...
        for orphan in orphans:
            issues.append({
                ...
                "details": {"fk_name": orphan.fk_name, "referred_table": orphan.referred_table}
            })
        ...
        for null in nulls:
            ...
            issues.append({
                ...
                "details": {"total": null['total_count'], "percent": null['percent']}
            })
        ...
        for dup in duplicates:
            issues.append({
                ...
                "details": {"columns": dup['columns'], "constraint": dup['constraint_name']}
            })
