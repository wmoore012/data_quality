...
def analyze_database_completeness(
    ...
    if total_columns > 0:
        overall_score = (
            sum(
                table.completeness_score * len(table.columns)
...
def identify_impossible_columns(database_url: str) -> dict[str, list[str]]:
    ...
    compiled_patterns = [
        re.compile(pattern, re.IGNORECASE) for pattern in impossible_patterns
    ]
...
def _analyze_table_completeness(
    engine: Engine, table_name: str, include_impossible_detection: bool = True
    ...
    try:
        ...
        for column_name, data_type in columns:
            ...
            if column_analysis:
                ...
                if (
                    include_impossible_detection
                    and column_analysis.is_likely_impossible
                ):
        ...
        if column_analyses:
            completeness_score = sum(
                col.fill_percentage for col in column_analyses
            ) / len(column_analyses)
        else:
...
def _analyze_column_completeness(
    engine: Engine, table_name: str, column_name: str, data_type: str, total_rows: int
    ...
    try:
        with engine.begin() as conn:
            ...
            result = conn.execute(
                text(f"SELECT COUNT(*) FROM {table_name} WHERE {column_name} = ''")
            )
...
def _generate_column_recommendations(
    ...
    if is_likely_impossible:
        recommendations.append(
            f"Consider if '{column_name}' is necessary - may be impossible to fill"
        )
    elif category == "critical":
        recommendations.append(
            f"URGENT: '{column_name}' has {null_percentage:.1f}% null values"
        )
        recommendations.append(f"Investigate data sources for '{column_name}'")
    elif category == "poor":
        recommendations.append(
            f"Improve data collection for '{column_name}' ({fill_percentage:.1f}% filled)"
        )
    elif category == "perfect":
        recommendations.append(
            f"'{column_name}' has excellent data quality - maintain current processes"
        )
...
def _generate_table_recommendations(
    ...
    avg_completeness = sum(col.fill_percentage for col in column_analyses) / len(
        column_analyses
    )
    if avg_completeness < 70:
        recommendations.append(
            f"Table '{table_name}' needs significant data quality improvement"
        )
    elif avg_completeness > 90:
        recommendations.append(
            f"Table '{table_name}' has excellent overall data quality"
        )
...
def _generate_summary_recommendations(
    ...
    if critical_columns > 0:
        recommendations.append(
            f"URGENT: Address {critical_columns} critical columns with >95% null values"
        )

    if impossible_columns > 0:
        recommendations.append(
            f"REVIEW: {impossible_columns} columns may be impossible to fill"
        )
    ...
    if table_analyses:
        ...
        recommendations.append(
            f"Best table: '{best_table.name}' ({best_table.completeness_score:.1f}%)"
        )
        recommendations.append(
            f"Worst table: '{worst_table.name}' ({worst_table.completeness_score:.1f}%)"
        )

    return recommendations


def _get_tables_for_analysis(
    ...
    try:
        ...
        if patterns:
            pattern_conditions = " OR ".join(
                [f"table_name LIKE '{pattern}'" for pattern in patterns]
            )
            query = text(
                f"""
                SELECT table_name
